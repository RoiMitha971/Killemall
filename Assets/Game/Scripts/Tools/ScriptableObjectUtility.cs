...
namespace Killemall.Tools
{
    public static class ScriptableObjectUtility
    {
        ...
        /// </summary>
        public static T CreateAsset<T>(string assetName, T asset) where T : ScriptableObject
        {
            if (asset == null)
            {
        ...
        public static void ShowDialog<T>(string defaultDestinationPath, Action<T> onScritpableObjectCreated = null) where T : ScriptableObject
        {
        ...
        private class ScriptableObjectSelector<T> : OdinSelector<Type> where T : ScriptableObject
        {
            ...
            {
                var scriptableObjectTypes = AssemblyUtilities.GetTypes(/*AssemblyTypeFlags.CustomTypes*/ AssemblyCategory.ProjectSpecific)
            ...
            private void ShowSaveFileDialog(IEnumerable<Type> selection)
            {
                ...
                dest = EditorUtility.SaveFilePanel("Save object as", dest, "[" + typeof(T).GetNiceName() + "] New", "asset");
